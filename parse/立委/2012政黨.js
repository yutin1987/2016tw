...
var parse = function (city) {
  var workbook = XLS.readFile('../../raw/立委/2012/政黨/立委-A05-6-(' + city + ').xls');
  ...
  while(sheet['D' + index]) {
    if (sheet['A' + index] && sheet['A' + index].v.trim()) {
      ...
      for (var x = 0; x < candidate.length; x++) {
        item[candidate[x]] = sheet[String.fromCharCode(68 + x) + index].v;
      }
  ...
  fs.writeFileSync('../../json/立委/2012/政黨/' + city + '.json', JSON.stringify({
    '政黨': candidate,
    '資料': data
  }, null, 2));
};
