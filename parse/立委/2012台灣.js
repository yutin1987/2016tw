...
var parse = function (city) {
  var workbook = XLS.readFile('../../raw/立委/2012/台灣/立委-A05-2-(' + city + ').xls');

  for (var j = 0; j < workbook.SheetNames.length; j++) {
    var sheet = workbook.Sheets[workbook.SheetNames[j]];
    var candidate = [];
    var i = 3;
    ...
    fs.writeFileSync('../../json/立委/2012/台灣/' + city + '-' + workbook.SheetNames[j] + '.json', JSON.stringify({
      '參選人': candidate,
      '資料': data
    }, null, 2));
  }
